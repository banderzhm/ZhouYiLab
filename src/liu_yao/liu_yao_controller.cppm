...
export import ZhouYi.LiuYaoAnalysis; // 对外暴露断卦请求与结果契约
...
using ZhouYi::LiuYaoAnalysis::AnalysisRequest;
using ZhouYi::LiuYaoAnalysis::AnalysisResult;
using ZhouYi::LiuYaoAnalysis::QuestionKind;
 ...
 *
 *
 * 排盘资料与断卦结论分别保存，调用方可独立展示或序列化，控制器不承担

 * * 中文排版职责。
 */
struct LiuYaoAnalysisResult {
  LiuYaoPaiPanResult pai_pan; ///< 完整排盘结果。
  AnalysisResult analysis;    ///< 依占问门类生成的断卦结果。
};
 ...
 *
 * @param
 * main_hexagram_code 主卦代码，按初爻至上爻排列。
 * @param bazi
 * 起卦时四柱，月支作月建、日支作日辰。
 * @param
 * changing_line_indices 动爻爻位列表（1—6）。
 * @param request
 * 占问门类、占问原文及可选的人工用神。

 * * @param generate_ai_json 是否同时生成原排盘的中文 JSON。
 */
LiuYaoAnalysisResult
analyze_liu_yao(const std::string &main_hexagram_code, const BaZi &bazi,
                const std::vector<int> &changing_line_indices,
                const AnalysisRequest &request, bool generate_ai_json = false);
