 ...
 */
LiuYaoPaiPanResult
calculate_liu_yao(const std::string &main_hexagram_code, const BaZi &bazi,
  ...
  // 调用内部排盘函数
  auto [yao_list, json_data] =
      sixYaoDivination(main_hexagram_code, bazi, changing_line_indices);
  ...
  // 返回结果
  return LiuYaoPaiPanResult{.yao_list = std::move(yao_list),
                            .json_data = std::move(json_data),
                            .ai_read_json_data = std::move(ai_read_json_data)};
}

LiuYaoAnalysisResult
analyze_liu_yao(const std::string &main_hexagram_code, const BaZi &bazi,
                const std::vector<int> &changing_line_indices,
                const AnalysisRequest &request, bool generate_ai_json) {
  auto pai_pan = calculate_liu_yao(main_hexagram_code, bazi,
                                   changing_line_indices, generate_ai_json);
  const auto main_info = get_hexagram_info(main_hexagram_code);

  std::optional<HexagramInfo> changed_info;
  if (!changing_line_indices.empty()) {
    auto changed_code = main_hexagram_code;
    for (const int position : changing_line_indices) {
      const auto index = static_cast<std::size_t>(position - 1);
      changed_code[index] = changed_code[index] == '0' ? '1' : '0';
    }
    changed_info = get_hexagram_info(changed_code);
  }

  auto analysis = ZhouYi::LiuYaoAnalysis::analyze(pai_pan.yao_list, main_info,
                                                  changed_info, bazi, request);
  return LiuYaoAnalysisResult{.pai_pan = std::move(pai_pan),
 ...
 * @brief 从爻辞生成主卦代码实现
 */
std::string yao_ci_to_hexagram_code(const std::vector<int> &yao_ci,
 ...
 * @brief 从数字卦生成主卦代码实现
 */
std::string numbers_to_hexagram_code(const std::vector<int> &numbers,
                                     bool odd_is_yang) {
  if (numbers.size() != 6) {
...
std::vector<LiuYaoPaiPanResult> batch_calculate_liu_yao(
    const std::vector<std::tuple<std::string, BaZi, std::vector<int>>>
        &requests,
    bool generate_ai_json) {
  ...
  for (const auto &[code, bazi, changing_lines] : requests) {
    try {
      results.push_back(
          calculate_liu_yao(code, bazi, changing_lines, generate_ai_json));
