...
public:
  ...
  [[nodiscard]] static auto pai_pan_solar(
    ...
    if (location) {
      ...
      minute = corrected.solar_time.get_minute();
    }
    // 参数验证
    if (month < 1 || month > 12) {
    ...
    try {
      ...
      return std::unexpected(
          fmt::format("从 tyme 模块获取信息失败: {}", e.what()));
    }
    ...
    auto result = QiMenPanGenerator::generate_pan(
        solar_term, tian_gan_day, di_zhi_day, tian_gan_hour, di_zhi_hour);
    ...
    if (result) {
      ...
      try {
        pan.ba_zi =
            BaZiBase::BaZi::from_solar(year, month, day, hour, minute, 0);
      } catch (...) {
   ...
   * }
   */
  [[nodiscard]] static auto pai_pan_lunar(int year, int month, int day,
    ...
    if (result) {
      ...
      try {
        pan.ba_zi =
            BaZiBase::BaZi::from_lunar(year, month, day, hour, minute, 0);
      } catch (...) {
        // 如果获取八字失败，保持为 nullopt
      }
    }
   ...
   * @brief 获取奇门盘的完整文本描述
   */
  [[nodiscard]] static auto get_pan_description(const QiMenPan &pan)
      -> std::string {
   ...
   * if (result) {
   *     std::string json_str =
   * QiMenController::get_pan_json_ordered(result.value()); std::println("{}",
   * json_str);
   * }
   */
  [[nodiscard]] static auto get_pan_json_ordered(const QiMenPan &pan)
      -> std::string {
   ...
   * @return JSON 对象
   */
  [[nodiscard]] static auto get_pan_json_object(const QiMenPan &pan)
      -> nlohmann::json {
    nlohmann::json j = pan;
    return j;
  }
  ...
  is_same_pan(const QiMenPan &pan1, const QiMenPan &pan2) noexcept {
    return pan1.dun == pan2.dun && pan1.yuan == pan2.yuan &&
           pan1.ju == pan2.ju && pan1.solar_term == pan2.solar_term;
  }
...
private:
   ...
   */
  [[nodiscard]] static auto build_ordered_json(const QiMenPan &pan)
    ...
    for (std::size_t i = 0; i < pan.palaces.size(); ++i) {
      ...
      palace_json["di_gan"] =
          std::string(Mapper::to_zh(static_cast<TianGan>(p.di_gan)));
      palace_json["tian_gan"] =
          std::string(Mapper::to_zh(static_cast<TianGan>(p.tian_gan)));
  ...
  convert_tyme_term_to_qimen(const tyme::SolarTerm &term) -> SolarTerm {
  ...
  calculate_hour_gan_zhi(std::uint8_t hour) noexcept
