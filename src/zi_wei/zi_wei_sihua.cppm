...
struct ZiHuaInfo {
  ...
  bool has_zi_hua() const { return !zi_hua_types.empty(); }

  bool has_zi_hua_type(SiHua type) const {
    return find(zi_hua_types.begin(), zi_hua_types.end(), type) !=
           zi_hua_types.end();
  }
...
struct FeiHuaChain {
  ...
  int get_depth() const { return static_cast<int>(chain.size()); }
...
public:
   ...
   */
  SiHuaSystem(const array<pair<TianGan, DiZhi>, 12> &gong_gan_zhi,
   ...
   */
  vector<FeiHuaChain> get_fei_hua_chains(int start_gong, SiHua si_hua_type,
...
private:
   ...
   * @brief 计算宫干四化
   */
  void calculate_gong_gan_si_hua(
      const array<pair<TianGan, DiZhi>, 12> &gong_gan_zhi);

  /**
   * @brief 递归查找飞化链
   */
  void find_chains_recursive(int current_gong, SiHua si_hua_type,
