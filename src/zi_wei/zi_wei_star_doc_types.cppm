// 紫微斗数星曜文档 - 类型契约
// 包含星曜类型、五行、阴阳等基础类型定义
export module ZhouYi.ZiWei.StarDoc.Types;
...
template <> struct ZhMap<XingYaoWuXing> {
  static constexpr auto get_map() {
    using namespace std::literals;
    return std::array{"金"sv, "木"sv, "水"sv, "火"sv, "土"sv};
  }
};

template <> struct ZhMap<XingYaoYinYang> {
  static constexpr auto get_map() {
    using namespace std::literals;
    return std::array{"阳"sv, "阴"sv};
  }
};

template <> struct ZhMap<StarType> {
  static constexpr auto get_map() {
    using namespace std::literals;
    return std::array{"十四主星"sv, "十四辅星"sv, "三十七杂曜"sv,
                      "四十八神煽"sv};
  }
};

template <> struct ZhMap<FuXingCategory> {
  static constexpr auto get_map() {
    using namespace std::literals;
    return std::array{"六吉星"sv, "六煽星"sv, "二助星"sv};
  }
};

template <> struct ZhMap<ZaYaoCategory> {
  static constexpr auto get_map() {
    using namespace std::literals;
    return std::array{"交际类"sv, "孤傲类"sv, "才艺类"sv,
                      "矜贵类"sv, "精神类"sv, "幸运类"sv,
                      "健康类"sv, "封赏类"sv, "思想类"sv};
  }
};
