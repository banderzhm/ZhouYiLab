 ...
 */
pair<int, int> get_kui_yue_index(TianGan year_gan, KuiYueSchool school) {
  // 地支索引映射（以寅宫为0）：
  ...
  case TianGan::Ding:
    return {9, 7}; // 亥酉
  case TianGan::Xin:
    return school == KuiYueSchool::QuanShuOne ? pair{0, 4} : pair{4, 0};
  case TianGan::Ren:
  case TianGan::Gui:
    return {1, 3}; // 卯巳
  default:
...
pair<int, int> get_huo_ling_index(DiZhi year_zhi, DiZhi hour_zhi) {
  ...
  if (year_zhi == DiZhi::Shen || year_zhi == DiZhi::Zi ||
      year_zhi == DiZhi::Chen) {
    huo_start = 0;  // 寅
    ling_start = 8; // 戌
  } else if (year_zhi == DiZhi::Yin || year_zhi == DiZhi::Wu ||
             year_zhi == DiZhi::Xu) {
    huo_start = 11; // 丑
    ling_start = 1; // 卯
  } else if (year_zhi == DiZhi::Si || year_zhi == DiZhi::You ||
             year_zhi == DiZhi::Chou) {
    huo_start = 1;  // 卯
    ling_start = 8; // 戌
  } else {          // 亥卯未
  ...
  return {fix_index(huo_start + hour_offset),
 ...
 * 由右弼之宫位起初一，逆行至生日安八座。
 */
pair<int, int> get_san_tai_ba_zuo_index(int lunar_month, int lunar_day,
                                        DiZhi hour_zhi) {
  auto [zuo_idx, you_idx] = get_zuo_you_index(lunar_month);

  // 晚子时需要加一天
  int day_offset =
      (hour_zhi == DiZhi::Zi && static_cast<int>(hour_zhi) >= 12) ? 1 : 0;
 ...
 */
pair<int, int> get_en_guang_tian_gui_index(int lunar_month, int lunar_day,
                                           DiZhi hour_zhi) {
 ...
 * 天寿由身宫起子，顺行至本生年支安之。
 */
pair<int, int> get_tian_cai_tian_shou_index(DiZhi year_zhi, int ming_index,
                                            int shen_index) {
  int zhi_idx = static_cast<int>(year_zhi);
...
int get_hua_gai_index(DiZhi year_zhi) {
  // 华盖口诀
  if (year_zhi == DiZhi::Yin || year_zhi == DiZhi::Wu ||
      year_zhi == DiZhi::Xu) {
    return 8; // 戌
  } else if (year_zhi == DiZhi::Shen || year_zhi == DiZhi::Zi ||
             year_zhi == DiZhi::Chen) {
    return 2; // 辰
  } else if (year_zhi == DiZhi::Si || year_zhi == DiZhi::You ||
             year_zhi == DiZhi::Chou) {
...
pair<int, int> get_gu_chen_gua_su_index(DiZhi year_zhi) {
  // 孤辰寡宿口诀
  int gu_chen, gua_su;
  if (year_zhi == DiZhi::Yin || year_zhi == DiZhi::Mao ||
      year_zhi == DiZhi::Chen) {
    gu_chen = 3; // 巳
    gua_su = 11; // 丑
  } else if (year_zhi == DiZhi::Si || year_zhi == DiZhi::Wu ||
             year_zhi == DiZhi::Wei) {
    gu_chen = 6; // 申
    gua_su = 2;  // 辰
  } else if (year_zhi == DiZhi::Shen || year_zhi == DiZhi::You ||
             year_zhi == DiZhi::Xu) {
    gu_chen = 9; // 亥
    gua_su = 5;  // 未
  } else {       // 亥子丑
...
int get_po_sui_index(DiZhi year_zhi) {
  ...
  int zhi_idx = static_cast<int>(year_zhi);
  constexpr array<int, 12> mapping = {3, 11, 7, 3, 11, 7, 3, 11, 7, 3, 11, 7};
...
pair<int, int> get_xun_kong_index(TianGan year_gan, DiZhi year_zhi) {
  const int gan_index = static_cast<int>(year_gan);
  const int zhi_index = static_cast<int>(year_zhi);
  const int xun_start_zhi = fix_index(zhi_index - gan_index, 12);

  // 空亡为本旬甲支之前的两个地支；宫位索引以寅为 0，故再减 2。
  int first = fix_index(xun_start_zhi - 4, 12);
  int second = fix_index(xun_start_zhi - 3, 12);

  // 混合排盘口径以阳干顺列、阴干逆列区分旬空与副旬。
  if (gan_index % 2 != 0)
    swap(first, second);
  return {first, second};
}
...
pair<int, int> get_jie_lu_kong_wang_index(TianGan year_gan) {
  ...
  auto [first, second] = mapping[static_cast<int>(year_gan) % 5];
  if (static_cast<int>(year_gan) % 2 != 0)
    swap(first, second);
  return {first, second};
}
 ...
 * - 阴男阳女：天伤在疾厄，天使在奴仆
 */
pair<int, int> get_tian_shi_tian_shang_index(int ming_index, bool is_male,
                                             DiZhi year_zhi) {
  // 迁移宫在命宫对宫
 ...
 * 病、死、墓、绝、胎、养。
 */
array<ChangSheng12, 12> arrange_chang_sheng_12(WuXingJu wu_xing_ju,
                                               bool is_male, DiZhi year_zhi) {
  ...
  for (int i = 0; i < 12; ++i) {
    int idx = shun_xing ? fix_index(start_idx + i) : fix_index(start_idx - i);
 ...
 * 飞廉、喜神、病符、大耗、伏兵、官府。
 */
array<BoShi12, 12> arrange_bo_shi_12(TianGan year_gan, DiZhi year_zhi,
                                     bool is_male) {
  array<BoShi12, 12> result{};

  constexpr array<BoShi12, 12> shun_xu = {
      BoShi12::BoShi,    BoShi12::LiShi,  BoShi12::QingLong, BoShi12::XiaoHao2,
      BoShi12::JiangJun, BoShi12::ZouShu, BoShi12::FeiLian2, BoShi12::XiShen2,
      BoShi12::BingFu2,  BoShi12::DaHao2, BoShi12::FuBing,   BoShi12::GuanFu2};
  ...
  for (int i = 0; i < 12; ++i) {
    int idx = shun_xing ? fix_index(lu_cun_idx + i) : fix_index(lu_cun_idx - i);
    result[idx] = shun_xu[i];
  }
