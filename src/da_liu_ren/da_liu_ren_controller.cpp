...
void DaLiuRenController::interactive_pai_pan() {
  ...
  DaLiuRenResult result =
      is_lunar ? DaLiuRenEngine::pai_pan_lunar(year, month, day, hour)
...
void DaLiuRenController::display_result_detailed(const DaLiuRenResult &result) {
  ...
  if (!result.ba_zi.xun_kong_1.empty()) {
    fmt::print("  旬空: {}{}\n", result.ba_zi.xun_kong_1,
               result.ba_zi.xun_kong_2);
  }
  fmt::print("\n");
  ...
  // 显示天地盘
  display_tian_di_pan(result.tian_di_pan, result.ba_zi.day.gan,
                      result.ba_zi.day.zhi);
...
void DaLiuRenController::display_tian_di_pan(const TianDiPan &tian_di_pan,
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    // 中文字符显示宽度为2，需要手动对齐
    fmt::print("    {}     {}      {}     {}  \n", di_str, tian_str,
               tian_dun_gan_str, tian_shen_jiang_name);
    // 第二行：显示地盘、地盘遁干、地盘神将
    fmt::print("           {}      {}     {}  \n", di_str, di_dun_gan_str,
               di_shen_jiang_name);
...
// 显示三传（完整版，包含六亲、天干、神将）
void DaLiuRenController::display_san_chuan(const SanChuan &san_chuan,
                                           TianGan day_gan, DiZhi day_zhi) {
  fmt::print("【三传】\n");
...
// 获取用户输入的日期
void DaLiuRenController::get_date_input(int &year, int &month, int &day,
                                        int &hour, bool &is_lunar) {
