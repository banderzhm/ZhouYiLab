...
/**
 * @brief 从名称末尾的五行中文字解析五行
 *
 *
 * 支持“木、火、土、金、水”本身，以及“大林木”“炉中火”等以五行
 *
 * 中文字结尾的专业名称。该函数只负责五行文本解析，不负责纳音取表。
 *
 * @param
 * name 待解析的中文名称
 * @return 名称末尾所表示的五行
 * @throws
 * std::invalid_argument 名称未以有效五行中文字结尾
 */
 ...
 */
inline ElementalRelation getElementalRelationship(const std::string &branch1,
  ...
  if (!fiveElementIndex.contains(elem1) || !fiveElementIndex.contains(elem2)) {
    return ElementalRelation::Error;
  }
...
inline std::string relationToString(ElementalRelation rel) {
  switch (rel) {
  case ElementalRelation::Generates:
    return "生";
  case ElementalRelation::GeneratedBy:
    return "被生";
  case ElementalRelation::Controls:
    return "克";
  case ElementalRelation::ControlledBy:
    return "被克";
  case ElementalRelation::Same:
    return "同";
  default:
    return "错误";
  }
 ...
 */
inline std::string getWangShuai(const std::string &lineElement,
                                const std::string &monthBranch) {
  // 验证输入有效性
  if (!branchFiveElements.contains(monthBranch) ||
      !fiveElementIndex.contains(lineElement)) {
    return "未知";
  }
  ...
  if (monthBranch == "辰" || monthBranch == "戌" || monthBranch == "丑" ||
      monthBranch == "未") {
    monthElement = "土"; // 当令者旺
  }
  ...
  if (lineIdx == monthIdx)
    return "旺"; // 同我者旺 (临月建)
  if (lineIdx == (monthIdx + 4) % 5)
    return "相"; // 生我者相 (月令生爻)
  if (lineIdx == (monthIdx + 1) % 5)
    return "休"; // 我生者休 (爻生月令)
  if (lineIdx == (monthIdx + 2) % 5)
    return "囚"; // 我克者囚 (爻克月令)
  if (lineIdx == (monthIdx + 3) % 5)
    return "死"; // 克我者死 (月令克爻)
