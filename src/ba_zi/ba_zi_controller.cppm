...
import ZhouYi.BaZiAnalysis; // 八字分析引擎
...
using ZhouYi::BaZiAnalysis::BirthContext;
 ...
 *
 *
 * 返回可解释的五行力量、日主强弱、承载能力及用神候选；

 * * `needs_review` 表示候选不应被误写为“没有用神”。
 */
 ...
 *
 * 出生时间默认取 BaZiResult
 * 中的精确时间，也可以在 request.birth_context
 *
 * 中覆盖；调用方只切换 method，不需要感知具体规则模块。
 *
 * 当前已接入子平法和盲派法；调用方仅通过 method 选择规则体系。

 */
AnalysisResult analyze_ba_zi(const BaZiResult &result,
                             const AnalysisRequest &request);

/** @brief Controller 的中文分析输出入口。 */
void display_analysis(const AnalysisResult &analysis);

/** @brief 以当前分析结果评价指定公历年的流年干支作用。 */
FortuneImpact analyze_liu_nian(const BaZiResult &result, int year,
                               const AnalysisConfig &config = {});

/** @brief 联合评价当前大运、指定流年及可选流月。 */
